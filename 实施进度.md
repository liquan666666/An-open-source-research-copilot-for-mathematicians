...
**实现的端点**（3个）：
- POST /ai/analysis/paper - 分析任意论文
- GET /ai/analysis/saved/{paper_id} - 分析已收藏的论文
- GET /ai/analysis/batch - 批量分析多篇论文
...
**代码统计**：
- 文件：`/apps/api/server/routes/ai_analysis.py`
- 代码行数：240 行
...
**实现的端点**（3个）：
- GET /ai/recommendations/papers - 推荐相关论文
- GET /ai/recommendations/tasks - 推荐学习任务
- GET /ai/recommendations/complete - 综合推荐
...
**代码统计**：
- 文件：`/apps/api/server/routes/ai_recommendations.py`
- 代码行数：330 行
...
**实现的端点**（4个）：
- POST /ai/roadmap/generate - 生成自定义路线图
- POST /ai/roadmap/save - 保存路线图到数据库
- POST /ai/roadmap/from-interest/{id} - 基于研究兴趣生成
- POST /ai/roadmap/auto-generate - 自动为所有兴趣生成
...
**代码统计**：
- 文件：`/apps/api/server/routes/ai_roadmap.py`
- 代码行数：310 行
...
**实现的端点**（5个）：
- GET /stats/overview - 全局统计概览
- GET /stats/timeseries - 时间序列数据（用于图表）
- GET /stats/achievements - 成就和徽章系统
- GET /stats/learning-progress - 学习进度详情
- GET /stats/activity-heatmap - 活动热力图
...
**代码统计**：
- 文件：`/apps/api/server/routes/stats.py`
- 代码行数：420 行
- Pydantic 模型：7 个
...
**实现的端点**（5个）：
- GET /export/json - 完整数据导出（JSON 格式）
- GET /export/csv/tasks - 任务导出（CSV 格式）
- GET /export/csv/checkins - 打卡导出（CSV 格式）
- GET /export/csv/papers - 论文导出（CSV 格式）
- GET /export/csv/interests - 研究兴趣导出（CSV 格式）
...
**代码统计**：
- 文件：`/apps/api/server/routes/export.py`
- 代码行数：300 行
...
**现有功能**：
- 支付路由已存在（`/apps/api/server/routes/payments.py`）
- 订阅管理已集成到用户系统
- 需要配置 Stripe API 密钥即可使用
...
**更新日期**：2026-01-19
**版本**：1.0.0-rc1
**状态**：所有后端功能完成！所有 4 个阶段 100% 完成。等待前端集成。
